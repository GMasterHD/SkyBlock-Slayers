		...
		if((Minecraft.getMinecraft()).ingameGUI instanceof GuiIngameForge && (e.type == RenderGameOverlayEvent.ElementType.EXPERIENCE || e.type == RenderGameOverlayEvent.ElementType.JUMPBAR)) {
			if(SkyBlockSlayers.saves.slayerVisible) {
				...
				for(int x = SkyBlockSlayers.config.Slayers.get(SkyBlockSlayers.index_configSelectedSlayer).Drops.size() - 1; x >= 0; --x) {
					...
					if(found) {
						fr.drawStringWithShadow(SkyBlockSlayers.config.Slayers.get(SkyBlockSlayers.index_configSelectedSlayer).Drops.get(x).name + ": " + save_drop.count, startX, currentY, ColorUtils.getColor(SkyBlockSlayers.config.Slayers.get(SkyBlockSlayers.index_configSelectedSlayer).Drops.get(x).rarity));
					} else {
						fr.drawStringWithShadow(SkyBlockSlayers.config.Slayers.get(SkyBlockSlayers.index_configSelectedSlayer).Drops.get(x).name + ": 0", startX, currentY, ColorUtils.getColor(SkyBlockSlayers.config.Slayers.get(SkyBlockSlayers.index_configSelectedSlayer).Drops.get(x).rarity));
					}
